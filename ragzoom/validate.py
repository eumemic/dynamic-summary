...
from ragzoom.store import TreeNode, Store
...
def validate_chunk_sizes(leaf_nodes: List[TreeNode], target_tokens: int, tolerance: float = 0.2) -> Optional[str]:
    ...
    if oversized:
        logger.warning(f"Found {len(oversized)} oversized chunks (>{max_allowed} tokens)")
        for node_id, tokens in oversized[:5]:  # Show first 5
            logger.warning(f"  {node_id}: {tokens} tokens")
    
    if undersized:
        logger.warning(f"Found {len(undersized)} undersized chunks (<{min_allowed} tokens)")
        for node_id, tokens in undersized[:5]:  # Show first 5
            logger.warning(f"  {node_id}: {tokens} tokens")
    
    if not oversized and not undersized:
        logger.info(f"✓ Chunk sizes validated: all within ±{int(tolerance*100)}% of {target_tokens} tokens")
    
    return None  # No errors


def validate_tree_structure(store: Store, document_id: str, original_text: Optional[str] = None) -> Optional[str]:
    ...
    for node in nodes:
        ...
        if node.span_start >= node.span_end:
            errors.append(f"Node {node.id}: Invalid span [{node.span_start}, {node.span_end})")
        ...
        if node.left_child_id or node.right_child_id:
            ...
            if node.left_child_id:
                ...
                if not left_child:
                    errors.append(f"Node {node.id}: Left child {node.left_child_id} not found")
            ...
            if node.right_child_id:
                ...
                if not right_child:
                    errors.append(f"Node {node.id}: Right child {node.right_child_id} not found")
            ...
            if node.left_child_id and node.right_child_id:
                ...
                if left_child and right_child:
                    if left_child.span_end < right_child.span_start:
                        gap = right_child.span_start - left_child.span_end
                        # If we have original text, check if gap is only whitespace
                        if original_text:
                            gap_text = original_text[left_child.span_end:right_child.span_start]
...
def validate_frontier_completeness(
    ...
    if sorted_segments[0][2] != document_span[0]:
        return f"Frontier starts at {sorted_segments[0][2]}, expected {document_span[0]}"
    
    # Check coverage ends at document end
    if sorted_segments[-1][3] != document_span[1]:
        logger.debug(f"Last segment: [{sorted_segments[-1][2]}, {sorted_segments[-1][3]})")
    ...
    logger.info(f"✓ Frontier completeness validated: {len(sorted_segments)} segments cover document")
    return None


def validate_no_overlap(frontier_segments: List[Tuple[str, str, int, int]]) -> Optional[str]:
    ...
    logger.info(f"✓ No overlap validated: all frontier segments are non-overlapping")
    return None
