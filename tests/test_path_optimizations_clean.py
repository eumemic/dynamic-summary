...
class TestPathOptimizations:
    ...
    def test_backward_compatibility_without_paths(self):
        """Test that TreeNavigator works when nodes don't have path fields."""
        # Create a store with nodes that have path=None to simulate old nodes
        store = SimpleMockStore()

        # Add nodes but then manually remove the path attribute to simulate legacy nodes
        store.add_node(
            node_id="parent",
            text="Parent node",
            span_start=0,
        ...
        # Remove path attribute to simulate legacy nodes
        delattr(store._nodes["parent"], "path")
        delattr(store._nodes["child"], "path")

        navigator = TreeNavigator(store.nodes)

        # These should still work using fallback methods
        ancestors = navigator.get_ancestors(["child"])
        assert len(ancestors) == 1
        assert ancestors[0].id == "parent"

        # Depth should still work via fallback
        depth = navigator.get_node_depth("child")
        assert depth == 1

        depth_parent = navigator.get_node_depth("parent")
        assert depth_parent == 0
