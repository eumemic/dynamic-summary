...
class TestChunkSizeRegression:
    ...
    def test_splitter_creates_correct_chunk_size(self, config):
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Allow some tolerance (±10%) due to boundary splitting
            min_tokens = int(config.leaf_tokens * 0.9)
            max_tokens = int(config.leaf_tokens * 1.1)
            
            # Last chunk can be smaller
            if i < len(chunks) - 1:
                assert min_tokens <= token_count <= max_tokens, \
                    f"Chunk {i} has {token_count} tokens, expected ~{config.leaf_tokens} (±10%)"
            else:
                assert token_count <= max_tokens, \
                    f"Last chunk {i} has {token_count} tokens, should be <= {max_tokens}"
        
        # Check average chunk size
        avg_tokens = sum(len(tokenizer.encode(chunk)) for chunk in chunks) / len(chunks)
        assert avg_tokens <= config.leaf_tokens * 1.1, \
            f"Average chunk size {avg_tokens} tokens is too large (expected ~{config.leaf_tokens})"
    ...
    async def test_indexed_chunks_have_correct_size(self, tmp_path, monkeypatch):
        ...
        test_doc = "The quick brown fox jumps over the lazy dog. " * 100  # ~900 tokens
        ...
        async def mock_chat_completion(*args, **kwargs):
            response = MagicMock()
            response.choices[0].message.content = f"Summary part 1 <<<MID>>> Summary part 2"
            return response
        ...
        for node in leaf_nodes:
            ...
            # Leaf nodes should be close to configured size
            min_tokens = int(config.leaf_tokens * 0.8)
            max_tokens = int(config.leaf_tokens * 1.2)
            
            assert min_tokens <= token_count <= max_tokens, \
                f"Leaf node {node.id} has {token_count} tokens, expected ~{config.leaf_tokens}"
