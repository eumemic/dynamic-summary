...
def calculate_strategy_statistics(results: list[dict]) -> pd.DataFrame:
    ...
    for strategy in df["strategy"].unique():
        ...
        stats.append({
            ...
            "within_10_pct": np.sum(abs_errors_pct <= 10) / len(abs_errors_pct) * 100,
            "within_20_pct": np.sum(abs_errors_pct <= 20) / len(abs_errors_pct) * 100,
...
def identify_compensation_factors(results: list[dict]) -> pd.DataFrame:
    ...
    for strategy in df["strategy"].unique():
        ...
        compensations.append({
            ...
            "recommendation": f"Ask for {compensation_factor:.2f}x the target" if abs(mean_error_pct) > 5 else "No compensation needed"
        })
...
def create_visualizations(results: list[dict], output_dir: str = "experiments/results"):
    ...
    sns.heatmap(pivot_data, annot=True, fmt=".1f", cmap="RdYlGn_r",
                vmin=0, vmax=50, ax=ax, cbar_kws={'label': 'Mean Absolute Error (%)'})
    ax.set_title("Performance Heatmap: Mean Absolute Error by Strategy and Compression Ratio")
    ...
    for i, strategy in enumerate(strategies):
        ...
        ax.scatter(strategy_df["target_tokens"], strategy_df["actual_tokens"],
                  alpha=0.5, s=20)

        # Add diagonal line
        max_val = max(strategy_df["target_tokens"].max(), strategy_df["actual_tokens"].max())
...
def generate_report(results_path: str = "experiments/results/raw_results.json"):
    ...
    if not strategy_stats.empty:
        ...
        report.append(f"- Mean absolute error: {best_strategy['mean_abs_error_pct']:.2f}%")
        report.append(f"- Within ±10%: {best_strategy['within_10_pct']:.1f}% of attempts")
        report.append(f"- Within ±20%: {best_strategy['within_20_pct']:.1f}% of attempts\n")
