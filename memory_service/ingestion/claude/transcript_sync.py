...
class SyncResult:
    ...
    new_last_uuid: str | None = None  # Last UUID after sync (for cursor update)
...
def execute_delta_sync(
    ...
    """Execute sync from ONLY the delta bytes (memory-efficient).

    This version processes only the new content, never loading full history.
    It uses the cursor's last_uuid to detect reverts.

    Args:
        session_id: The session ID (used as document_id)
        delta: New JSONL content as bytes (NOT the full content)
        cursor: SessionCursor with byte_offset, last_uuid, span_end
        client: RagZoom client with append() and truncate() methods
    ...
    cursor_last_uuid: str | None = getattr(cursor, "last_uuid", None)
    cursor_span_end: int = getattr(cursor, "span_end", 0)

    logger.info(
        "[TIMING] execute_delta_sync start: delta_bytes=%d cursor_last_uuid=%s",
        len(delta),
        cursor_last_uuid[:8] if cursor_last_uuid else None,
    )

    # Parse only the delta to get new messages
    delta_head = _get_current_head_from_bytes(delta)
    if delta_head is None:
        # Empty delta, nothing to do
        return SyncResult(
            document_id=document_id,
            truncated=False,
            truncate_span=None,
            appended_uuids=[],
            new_span_end=cursor_span_end,
            new_last_uuid=cursor_last_uuid,
        )

    # Build parent map from delta only
    delta_parent_map = _build_parent_map_from_bytes(delta)
    t_parse = time.perf_counter()

    # Check if delta connects to our last known state (revert detection)
    # The first message in delta should have parentUuid == cursor_last_uuid
    # OR be a root message (parentUuid is None) if this is the first sync
    first_uuid = _get_first_uuid_from_bytes(delta)
    first_parent = delta_parent_map.get(first_uuid) if first_uuid else None

    truncated = False
    truncate_span: int | None = None

    if cursor_last_uuid is not None:
        # We have previous state - check for revert
        if first_parent != cursor_last_uuid:
            # Revert detected - the delta doesn't connect to our last known message
            # Signal that client needs to re-sync from scratch
            logger.warning(
                "Revert detected: delta first_parent=%s != cursor_last_uuid=%s",
                first_parent[:8] if first_parent else None,
                cursor_last_uuid[:8],
            )
            # Truncate document and reset
            truncate_method = getattr(client, "truncate")
            truncate_method(document_id, 0)
            truncated = True
            truncate_span = 0
            # Return indicating truncation - servicer will reset cursor
            return SyncResult(
                document_id=document_id,
                truncated=True,
                truncate_span=0,
    ...
    # Normal append - process all messages in delta
    # Get all UUIDs from delta in order
    all_uuids: list[str] = []
    for record, _ in _iter_jsonl_from_bytes(delta):
        uuid = record.get("uuid")
        if isinstance(uuid, str) and not record.get("isCompactSummary"):
    ...
    records_by_uuid = _build_records_map_from_bytes(delta, set(all_uuids))
    t_records = time.perf_counter()
    ...
    for uuid in all_uuids:
        msg_record = records_by_uuid.get(uuid)
        if msg_record is None:
            continue

        is_user_message = (
            msg_record.get("type") == "user" and "toolUseResult" not in msg_record
        )
    ...
    t_transcribe = time.perf_counter()
    logger.info(
        "[TIMING] delta parse: %.3fs records: %.3fs transcribe: %.3fs segments=%d",
        t_parse - t0,
        t_records - t_parse,
        t_transcribe - t_records,
        len(segment_texts),
    ...
    if segment_texts:
        batch_append_method = getattr(client, "batch_append")
        result = batch_append_method(document_id, segment_texts)
        new_span_end = getattr(result, "span_end", cursor_span_end)
        t_append = time.perf_counter()
        logger.info(
            "[TIMING] batch_append: %.3fs segments=%d",
            t_append - t_transcribe,
            len(segment_texts),
        )

    # Update new_last_uuid to the last message we processed
    if appended_uuids:
        new_last_uuid = appended_uuids[-1]

    t_end = time.perf_counter()
    logger.info(
        "[TIMING] execute_delta_sync complete: total=%.3fs appended=%d",
        t_end - t0,
        len(appended_uuids),
    )
    ...
    return SyncResult(
        ...
        appended_uuids=appended_uuids,
        new_span_end=new_span_end,
        new_last_uuid=new_last_uuid,
    )


def _get_first_uuid_from_bytes(content: bytes) -> str | None:
    """Get the UUID of the first non-compaction message from bytes."""
    for record, _ in _iter_jsonl_from_bytes(content):
        if record.get("isCompactSummary"):
            continue
        uuid = record.get("uuid")
