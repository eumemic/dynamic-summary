...
class TestWhitespaceReconstruction:
    ...
    def test_whitespace_gap_reconstruction(self, setup):
        ...
        assert reconstructed == test_text, f"Reconstructed text doesn't match original.\nExpected: {repr(test_text)}\nGot: {repr(reconstructed)}"

        # Verify no character was lost
        assert len(reconstructed) == len(test_text), f"Length mismatch: expected {len(test_text)}, got {len(reconstructed)}"
    ...
    def test_indexing_with_whitespace_gaps(self, setup):
        ...
        with store.SessionLocal() as session:
            ...
            for i in range(len(leaf_nodes) - 1):
                ...
                assert current_end == next_start, f"Gap found between nodes: {current_end} to {next_start}"
