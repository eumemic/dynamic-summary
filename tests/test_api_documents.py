...
class TestDocumentAPI:
    ...
    def test_list_documents_with_data(self, client):
        ...
        # Index two documents
        response1 = client.post("/index", json={
            "text": "Dragons are mighty creatures.",
            "document_id": "dragons-doc"
        })
        assert response1.status_code == 200

        response2 = client.post("/index", json={
        ...
        # Find each document
        dragons_doc = next(d for d in data["documents"] if d["document_id"] == "dragons-doc")
        wizards_doc = next(d for d in data["documents"] if d["document_id"] == "wizards-doc")
    ...
    def test_query_with_document_isolation(self, client):
        ...
        # Index two documents
        response1 = client.post("/index", json={
            "text": "Dragons breathe fire and fly.",
            "document_id": "dragons"
        })
        assert response1.status_code == 200
        ...
        response = client.post("/query", json={
            "query": "Tell me about fire breathing",
            "document_id": "dragons"
        })
        assert response.status_code == 200
        data = response.json()
