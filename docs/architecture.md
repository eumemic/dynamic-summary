...
-   **Parent Nodes (Depth > 0):** Each parent node is a summary of its two children. This summary is generated by an LLM with a `<<<MID>>>` delimiter marking where the right child's summary begins. The delimiter is stored in the database but removed before presenting to users. This process is applied recursively, creating a hierarchical summary of the entire document, with the root node representing a synopsis of the whole text.
