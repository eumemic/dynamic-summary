...
class TelemetryExplorer:
    ...
    def _browse_nodes(self) -> None:
        ...
        while True:
            ...
            if action == "q":
                ...
                idx = Prompt.ask("Enter node index", default=str(start_idx))
                try:
                    self._show_specific_node_details(df.iloc[int(idx)])
    ...
    def _find_outliers(self) -> None:
        ...
        if "input_amplification" in self.nodes_df.columns:
            high_amp = self.nodes_df[self.nodes_df["input_amplification"] > 3.0]
            if len(high_amp) > 0:
                console.print(f"[yellow]Found {len(high_amp)} nodes with input amplification > 3.0x:[/yellow]")
        ...
        if "retry_count" in self.nodes_df.columns:
            high_retry = self.nodes_df[self.nodes_df["retry_count"] > 1]
            if len(high_retry) > 0:
        ...
        if "input_amplification" in self.nodes_df.columns:
            ...
            if len(amp_df) > 0:
                ...
                table.add_row(
                    ...
                    f"{amp_df.get('output_amplification', pd.Series()).mean():.2f}x" if 'output_amplification' in amp_df else "-"
                )
                table.add_row(
                    "Median",
                    f"{amp_df['input_amplification'].median():.2f}x",
                    f"{amp_df.get('output_amplification', pd.Series()).median():.2f}x" if 'output_amplification' in amp_df else "-"
                )
                table.add_row(
                    "P90",
                    f"{amp_df['input_amplification'].quantile(0.9):.2f}x",
                    f"{amp_df.get('output_amplification', pd.Series()).quantile(0.9):.2f}x" if 'output_amplification' in amp_df else "-"
                )
