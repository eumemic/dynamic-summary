...
class TestTurnLevelAppendEntryTracking:
    ...
    def test_each_turn_creates_one_append_unit(self, tmp_path: Path) -> None:
        ...
        # All turns should be in appended_uuids
        assert "msg1" in result.appended_uuids
        assert "msg2" in result.appended_uuids
        assert "msg3" in result.appended_uuids
        assert "msg4" in result.appended_uuids
        assert "msg5" in result.appended_uuids
    ...
    def test_tool_results_grouped_in_same_turn(self, tmp_path: Path) -> None:
        ...
        # Tool result UUID (msg3) is excluded from appended_uuids
        assert "msg1" in result.appended_uuids
        assert "msg2" in result.appended_uuids
        assert "msg3" not in result.appended_uuids  # Tool result excluded
        assert "msg4" in result.appended_uuids
    ...
    def test_incremental_sync_adds_new_turn(self, tmp_path: Path) -> None:
        ...
        assert len(client.batch_append_calls) == 1
        assert "msg1" in result1.appended_uuids
        assert "msg2" in result1.appended_uuids
        ...
        # First turn should not be re-appended
        assert "msg1" not in result2.appended_uuids
        assert "msg2" not in result2.appended_uuids
