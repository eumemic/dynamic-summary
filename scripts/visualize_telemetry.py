...
class TelemetryVisualizer:
    ...
    def load_benchmark_data(self, file_path: Path) -> dict:
    ...
    def _create_config_from_metrics(self, metrics: dict) -> RagZoomConfig:
        """Create a config object from metrics data for cost calculations."""
        # Extract cost configuration from metrics if available
        efficiency = metrics.get("efficiency", {})
        
        # Estimate costs from the data
        # Default to typical GPT-4o-mini and text-embedding-3-small costs
        return RagZoomConfig(
            openai_api_key="dummy",  # Not needed for analysis
            embedding_cost_per_1k=0.0001,  # text-embedding-3-small
            ...
            if stats["count"] > 0:
                ...
                for bucket, data in stats["histogram"].items():
                    ...
                    if count > 0:
                        ...
                        if bucket == "0-10%":
                            deviations = [5] * count  # Use midpoint
                        elif bucket == "10-25%":
    ...
    def _plot_token_heatmap(self, telemetry: dict, ax: plt.Axes) -> None:
        ...
        try:
            ...
            for node_type in types:
                row = []
                for level in levels:
                    tokens = token_data[level].get(node_type, [])
    ...
    def _generate_markdown_report(self, data: dict, telemetry: dict, config: RagZoomConfig, chunk_size: int) -> None:
...
"""
    ...
    def _plot_cost_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
    ...
    def _plot_throughput_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
    ...
    def _plot_amplification_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
    ...
    def _plot_batch_efficiency_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
    ...
    def _plot_retry_rate_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
        ...
        # Add warning line at 20%
        ax.axhline(y=20, color='red', linestyle='--', alpha=0.5, label='20% threshold')
        ax.legend()
    
    def _plot_accuracy_comparison(self, benchmarks: dict[int, dict], ax: plt.Axes) -> None:
        ...
        target_sizes = sorted([int(t) for t in target_sizes if t.isdigit()])
        x = np.arange(len(chunk_sizes))
        width = 0.8 / len(target_sizes)
