...
class TestBudgetGuarantee:
    ...
    def test_conservative_n_max_calculation(self, setup_system):
        ...
        for budget in test_budgets:
            ...
            if budget < config.leaf_tokens * safe_factor:
                ...
                assert conservative_n_max == 1, f"Should return 1 for small budget {budget}"
            else:
                # Verify this n_max would never exceed budget with safe factor
                worst_case_tokens = conservative_n_max * config.leaf_tokens * safe_factor
                ...
                assert conservative_n_max == expected_n_max, \
    ...
    def test_worst_case_parent_grandchild_3x_bound(self, setup_system):
        ...
        # Add leaves
        store.add_node("0_0_200_leaf1", leaf1_text, [0.1] * 384, 0, 0, 200, None, None, None, "test-doc")
        store.add_node("0_200_400_leaf2", leaf2_text, [0.2] * 384, 0, 200, 400, None, None, None, "test-doc")
        store.add_node("0_400_600_leaf3", leaf3_text, [0.3] * 384, 0, 400, 600, None, None, None, "test-doc")
        store.add_node("0_600_800_leaf4", leaf4_text, [0.4] * 384, 0, 600, 800, None, None, None, "test-doc")
        ...
        store.add_node("1_0_400_parent1", parent1_text, [0.15] * 384, 1, 0, 400, None, None, None, "test-doc",
                      mid_offset=len("Summary of leaf one. "))
        store.add_node("1_400_800_parent2", parent2_text, [0.35] * 384, 1, 400, 800, None, None, None, "test-doc",
                      mid_offset=len("Summary of leaf three. "))

        # Create grandparent
        grandparent_text = "Overall summary left. <<<MID>>> Overall summary right."
        store.add_node("2_0_800_root", grandparent_text, [0.25] * 384, 2, 0, 800, None, None, None, "test-doc",
        ...
        with store.SessionLocal() as session:
            ...
            for leaf_id, parent_id in [("0_0_200_leaf1", "1_0_400_parent1"),
        ...
        result.frontier_nodes = ["1_0_400_parent1", "0_400_600_leaf3"]  # Parent + non-child leaf
        ...
        assembled_text, token_count = assembler.assemble_with_budget(result, token_budget=700)
    ...
    def test_drop_strategy_vs_truncate(self, setup_system):
        ...
        summary_drop, tokens_drop = assembler.assemble_with_budget(result_drop, token_budget=small_budget)

        # Verify drop strategy stays within budget
        assert tokens_drop <= small_budget, f"Drop strategy exceeded budget: {tokens_drop} > {small_budget}"
        ...
        summary_truncate, tokens_truncate = assembler.assemble_with_budget(result_truncate, token_budget=small_budget)

        # Verify truncate strategy stays within budget
        assert tokens_truncate <= small_budget, f"Truncate strategy exceeded budget: {tokens_truncate} > {small_budget}"
    ...
    def test_drop_strategy_maintains_slope_cap(self, setup_system):
        ...
        store.add_node("0_0_200_leaf", leaf1_text, [0.1] * 384, 0, 0, 200, None, None, None, "test-doc")
        store.add_node("1_200_400_bridge", bridge_text, [0.2] * 384, 1, 200, 400, None, None, None, "test-doc")
        store.add_node("2_400_600_deep", deep_text, [0.3] * 384, 2, 400, 600, None, None, None, "test-doc")
        ...
        result = RetrievalResult(
            ...
            scores={"0_0_200_leaf": 0.9, "1_200_400_bridge": 0.3, "2_400_600_deep": 0.8},  # Bridge has low score
            coverage_map={"0_0_200_leaf": True, "1_200_400_bridge": True, "2_400_600_deep": True},
        ...
        config.budget_strategy = "drop"
        small_budget = 450  # Should keep leaf (200) + deep (300) = 500, but drop bridge (100)

        # Assemble with drop strategy
        assembled_text, token_count = assembler.assemble_with_budget(result, token_budget=small_budget)

        # Should not exceed budget
        assert token_count <= small_budget, f"Budget exceeded: {token_count} > {small_budget}"
