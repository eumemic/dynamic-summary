...
@pytest.mark.asyncio
async def test_batch_append_text_passes_summarization_guidance() -> None:
    """Verify servicer extracts summarization_guidance from BatchAppendTextRequest.

    Spec: specs/transcript-summarization-guidance.md § Phase 1
    Test: tests/test_indexer_servicer_sqlite.py::test_batch_append_text_passes_summarization_guidance
    """
    backend = SQLiteStorageBackend()
    try:
        index_config = IndexConfig.load()
        ...
        document_id = "doc-batch-guidance"
        custom_guidance = (
            "This is a conversation transcript. "
            "Preserve identity, decisions, and causality."
        )

        outcome = AppendOutcome(
            document_id=document_id,
            appended_span_start=0,
            appended_span_end=20,
            new_leaf_ids=["leaf-1", "leaf-2"],
            deleted_node_ids=[],
            total_leaves=2,
        )
        ...
        # Create BatchAppendTextRequest with summarization_guidance
        request = pb2.BatchAppendTextRequest(
            document_id=document_id,
            units=[
                pb2.AppendUnit(content=b"Turn 1: User asks a question."),
                pb2.AppendUnit(content=b"Turn 2: Assistant responds."),
            ],
            collect_telemetry=False,
            summarization_guidance=custom_guidance,
        )

        await servicer.BatchAppendText(request, StubContext())

        # Verify document was created with custom guidance stored
        doc = backend.get_document_by_id(document_id)
        assert doc is not None, "Document should be created"
        assert (
